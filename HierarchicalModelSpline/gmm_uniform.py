...
class GMM_UNIFORM(object):
    """
    This class fits a Gaussian Mixture Model together with a uniform density to the data
    """
    def __init__(self,data,nComp=2,tol=1e-4,maxIter=1000,n_tri=10):
        ...
        for G in nComp :
            ...
            while veritas and cTri <= n_tri :
                ...
                # print sigma
                #---k-means -----------
                # km     = cluster.KMeans(n_clusters=G,max_iter=1000,n_init=1,tol=tol)
                # km.fit(data)
                # mu     = km.cluster_centers_

                # for i in range(G):
                #     idx    = np.where(km.labels_==i)[0]
                #     pro[i] = len(idx)/float(N)
                #     if pro[i] <=0:
                #         sys.exit("Zero in fraction")
                #     sigma[i] = covariance.empirical_covariance(data[idx,:])
                #     eigs = scipy.linalg.eigvals(sigma[i])
                #     if np.any(eigs<0):
                #         print("Setting covariance to identity matrix.")
                #         sigma[i] = np.eye(D)
                ...
                isgs  = np.zeros((G-1,D,D))
                div   = np.zeros(G-1)
                for i in range(G-1):
                        isgs[i]    = inv(sigma[i])
                        div[i]     = 0.5*np.log(((2*np.pi)**(D))*np.linalg.det(sigma[i]))

                for r in range(N):
                    lden[0]     = np.log(pro[0])+lunden
                    for i in range(G-1):
                        mvn     = -0.5*np.dot((data[r]-mu[i]),np.dot(isgs[i],(data[r]-mu[i]).T))
                        lden[i+1] = np.log(pro[i+1])+mvn-div[i]
           ...
           #----- DO fit ---------- 
            ...
            fit    = OneModel(data,G,z,pro,mu,sigma,lunden,tol,maxIter)
...
class OneModel(object):
        ...
        def __init__(self,data,G,z,pro,mu,sigma,lunden,tol,maxIter):
            N,D    = data.shape
            ollik  = -1
...
# X2 = np.random.multivariate_normal(mu1, sg,n2)
# X3 = np.random.multivariate_normal(mu2, sg,n3)
# X4 = np.random.multivariate_normal(mu3, sg,n4)
# X5 = np.random.multivariate_normal(mu4, sg,n5)

# X  = np.vstack((X1,X2,X3,X4,X5))

# #-------------------
# fit = GMM_UNIFORM(X,[5],tol=1e-5)
