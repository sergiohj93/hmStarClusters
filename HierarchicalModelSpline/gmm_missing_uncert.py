...
class GMM_MISSING_MPI(object):
    ...
    def __init__(self,data,uncert,nComp=2,tol=1e-4,maxIter=1000,rho=0.5,tolCov=1e-2,n_tri=1,
        ...
        for G in nComp :
            ...
            while veritas and cTri <= n_tri :
                ...
                for r in range(my_N):
                    ...
                    w = isobs[r,]
                    for i in range(G):
                        sg      = sigma[i][np.ix_(w,w)]+my_sigma[r][np.ix_(w,w)]
           ...
           #----- DO fit ---------- 
            comm.barrier()
            fit    = OneModelMPI(my_data,my_sigma,G,my_z,pro,mu,sigma,isobs,tol,maxIter,rho,tolCov)
...
class OneModelMPI(object):
        ...
        def __init__(self,my_data,my_sigma,G,my_z,pro,mu,sigma,isobs,tol,maxIter,rho,tolCov):
