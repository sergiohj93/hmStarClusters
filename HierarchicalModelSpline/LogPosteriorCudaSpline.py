...
from LikelihoodCuda import KernelLikelihood
...
class LogPosteriorModule(object):
    ...
    def computeLikelihood(self, ctx):
        ...
        lpsc     = np.zeros((len(self.obs_c),3))
        lpsi     = np.zeros((len(self.obs_i),3))
        

        #Call to a cuda code that calculates the likelihoods. Return lpsc and lpsi.
        KernelLikelihood[griddim, blockdim](self.obs_c,self.obs_i,pi,pi_pm_ps,pi_pm_bs,
                pi_color,mu_color,vr_color,
                mu_pm_ps,sg_pm_ps,mu_pm_bs,sg_pm_bs,
                coefs,phot_cov,alpha,
                self.stp,self.rg_color,
                evals,self.degspl,id_mag,lpsc,lpsi) 
        ...
        # PC1   = '\t'.join([str(seq[1])  for seq in lpsc])
        # PC2   = '\t'.join([str(seq[2])  for seq in lpsc])
        ...
        sys.exit()
        return lpp+np.sum(lps_c)+np.sum(lps_i)
