...
def p_color_p(color,pi,mu,vr,rg):
    # There are for normal distributions, they are truncated.
    #-------- denominators without sqrt(pi) ------
    de0 = np.sqrt(2*vr[0])
    de1 = np.sqrt(2*vr[1])
    ...
    de4 = np.sqrt(2*vr[4])
    #-------- exponentials divided by denominators----
    ps0 = np.exp(-((color-mu[0])**2)/(2*vr[0]))/de0
    ps1 = np.exp(-((color-mu[1])**2)/(2*vr[1]))/de1
    ps2 = np.exp(-((color-mu[2])**2)/(2*vr[2]))/de2
    ps3 = np.exp(-((color-mu[3])**2)/(2*vr[3]))/de3
    ps4 = np.exp(-((color-mu[4])**2)/(2*vr[4]))/de4
    #-------- truncation factors, wihtout 0.5------
    tr0 = math.erf((rg[1]-mu[0])/de0)-math.erf((rg[0]-mu[0])/de0)
    tr1 = math.erf((rg[1]-mu[1])/de1)-math.erf((rg[0]-mu[1])/de1)
    tr2 = math.erf((rg[1]-mu[2])/de2)-math.erf((rg[0]-mu[2])/de2)
    ...
    ps  = pi[0]*(ps0/tr0) + pi[1]*(ps1/tr1) + pi[2]*(ps2/tr2) + pi[3]*(ps3/tr3) + pi[4]*(ps4/tr4)
    #return value with common factors 0.5 and sqrt(pi)
    ps  = (2/np.sqrt(np.pi))*ps
    #-------
    # ps  = 0.99*(1.0/(rg[1]-rg[0]))+0.01*ps
    return ps
...
def MarginalColor(phot,sg_obs,sg_cls,alpha,coefs,pi_color,mu_color,vr_clr,
    ...
    Inverse5x5(sg,isg)
    ###################### SINGLE MASS STARS ####################################################
    #sub1 = cuda.local.array(shape=max2d, dtype=float32)
    sub1 = np.empty(max2dEvals)
    ...
    #ps = cuda.local.array(shape=max1d, dtype=float32)
    ps = np.empty(max1d)
    ps = ps[0:len(tau)]
    #bs = cuda.local.array(shape=max1d, dtype=float32)
    bs = np.empty(max1d)
    bs = bs[0:len(taub)]
...
#@jit
def MarginalColorInc(phot_full,sg_obs_full,sg_cls_full,alpha_full,coefs,pi_color,
                          mu_color,vr_clr,rg_color,stp,idx,evals,degspl,id_mag):
    dclr = 0.1
    dmha = 3.5
                    
    #------- missing ---------
    #alpha = cuda.local.array(shape=max1d, dtype=float32)
    alpha = np.empty(max1d)
...
'''def MarginalColorInc(phot,sg_obs,sg_cls,alpha,coefs,pi_color,mu_color,vr_clr,
                    rg_color,stp,w,evals,degspl):
    
    idx      = np.ix_(w)
    
    alpha    = alpha[idx]
    
    getItems1d(phot,w,phot)
    phot = phot[0:len(w)]

    getItems2d(sg_obs,w,w,sg_obs)
    sg_obs = sg_obs[0:len(w),0:len(w)]

    getItems2d(sg_cls,w,w,sg_cls)
    sg_cls = sg_cls[0:len(w),0:len(w)]
    ...
    #sub1 = cuda.local.array(shape=max2d, dtype=float32)
    sub1 = np.empty(max2dEvals)
    sub1 = sub1[0:len(evals),0:len(w)]
    getItems2d(evals,range(len(evals)),w,sub1)
    subMatVec(sub1,phot,sub1)
    ...
    #ctr_idx = cuda.local.array(shape=max1d, dtype=float32)
    ctr_idx = np.empty(max1d)
    ctr_idx = ctr_idx[0:len(w)]
    getItems1d(ctr,w,ctr_idx)
    #sub2 = cuda.local.array(shape=max2d, dtype=float32)
    sub2 = np.empty(max2d)
    sub2 = sub2[0:len(bins),0:len(w)]
    getItems2d(bins,range(len(bins)),w,sub2)
    subMatVec(sub2,ctr_idx,sub2)
    minBall,maxBall = CalculateBalls(isg, bins, 3.5, sub2)
    #dom = cuda.local.array(shape=(2), dtype=float32)
    dom = np.empty(2)
    dom[0] = min(ctr[id_clr]-0.05,minBall)
    dom[1] = max(ctr[id_clr]+0.05,maxBall)
    ...
    # bbins = evals.copy()
    # #---- true binaries phot
    subItems2d(bbins,range(len(bbins)),id_mag,dm)
    ...
    #sub3 = cuda.local.array(shape=max2d, dtype=float32)
    sub3 = np.empty(max2d)
    sub3 = sub3[0:len(bbins),0:len(w)]
    getItems2d(bbins,range(len(bbins)),w,sub3)
    subMatVec(sub3,phot,sub3)
    ctrb_i = CalculateCtr_i(isg, sub3)
    #ctrb = cuda.local.array(shape=max1d, dtype=float32)
    ctrb = np.empty(max1d)
    ...
    #ctrb_idx = cuda.local.array(shape=max1d, dtype=float32)
    ctrb_idx = np.empty(max1d)
    ctrb_idx = ctrb_idx[0:len(w)]
    getItems1d(ctrb,w,ctrb_idx)
    #sub4 = cuda.local.array(shape=max2d, dtype=float32)
    sub4 = np.empty(max2d)
    sub4 = sub4[0:len(bbinsR),0:len(w)]
    getItems2d(bbinsR,range(len(bbinsR)),w,sub4)
    subMatVec(sub4,ctrb_idx,sub4)
    minBall,maxBall = CalculateBalls(isg, bbinsR, 3.5, sub4)
    #domb = cuda.local.array(shape=(2), dtype=float32)
    domb = np.empty(2)
    domb[0] = min(ctrb[id_clr]-0.05,minBall)
    domb[1] = max(ctrb[id_clr]+0.05,maxBall)
    ...
    bs = np.empty(max1d)
    bs = bs[0:len(taub)]
    LikeMemberPHInc(phot,sg_obs,sg_cls,isg,alpha,tau,taub,coefs,rg_color,w,degspl,ps,bs)
    PhPs  = dotVec(pclr,ps)*dtau
    PhBs  = dotVec(pclrb,bs)*dtaub
    
    return PhPs,PhBs'''

def logLikeIndependent(datum,pi,pi_pm_ps,
    pi_pm_bs,pi_color,mu_color,vr_clr,
...
def logLikeIndependentInc(datum,pi,pi_pm_ps,
    pi_pm_bs,pi_color,mu_color,vr_clr,
    mu_pm_ps,sg_pm_ps,mu_pm_bs,sg_pm_bs,
    coefs,phot_cov,alpha,
    stp,rg_color,evals,degspl,id_mag):
